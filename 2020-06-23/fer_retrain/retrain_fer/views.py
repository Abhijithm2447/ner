        ...
        if flag:  
            ...
            for entity in entity_list:
                ...
                if entity_form.is_valid():                    
                    ...
                    obj_cur_data = EntityDB.objects.filter(query=query,start_pos=int(entity[0]),end_pos=int(entity[1])).order_by('-id')  
                ...
                if entity_form.errors:
                    ...
                    obj_cur_data = EntityDB.objects.filter(query=query,start_pos=int(entity[0]),end_pos=int(entity[1])).order_by('-id')  
                    for obj in obj_cur_data:                
                        # tmp_entity_data_id = obj.id
                        entity_form = EntityDBForm(entity_data, instance = obj)
                        if entity_form.is_valid():
                            entity_form.save(commit=True)
                            result.append("updated successfully")
                        if entity_form.errors:
                            error.append("cant update data")
                    status = 200
        f_result = {
            'result' : result,
            'error' : error
...
def Train(TRAIN_DATA, model=None, new_model_name='ORG', output_dir=None, n_iter=20):
    ...
    obj_cur_data = EntityDB.objects.all()  
    LABELS = set()
    for obj in obj_cur_data:
        LABELS.add(obj.entity_name)
    # pdb.set_trace()
...
#______________________________________________________________________________________________________
@method_decorator(csrf_exempt, name='dispatch')
class DeleteNotTrainedData(HttpresponseMixin, SerializerMixin, View):    
    def assure_path_exists(self, path):
        dir = os.path.dirname(path)
        if not os.path.exists(dir):
            os.makedirs(dir)
    ...
    def post(self, request, *args, **kwargs):
        ...
        if 'delete' in request.POST:
            ...
            flag = False
            error.append("delete is not given")          
        if 'n_iter' in request.POST:
            n_iter = int(request.POST['n_iter'])
        if delete == "True" or delete == 1:            
...
class HardTrainData(HttpresponseMixin, SerializerMixin, View):
    def update_training_status(self, obj_entity, status):
        entity_data = {  
            ...
            'entity_name' : obj_entity.entity_name, #entity_list[i+2],
    ...
    def post(self, request, *args, **kwargs):
        ...
        if train == "True" or train == '1':
            ...
            for obj_query in obj_QueryDB: 
                ...
                for obj_ent in obj_EntityDB: 
                    if prev_end_index <= obj_ent.start_pos:
                        if obj_ent.start_pos != obj_ent.end_pos:
                            ent.append((obj_ent.start_pos, obj_ent.end_pos, obj_ent.entity_name))  
                            prev_end_index = obj_ent.end_pos
                    
                    # change training status
                    self.update_training_status(obj_entity = obj_ent, status=True)
                ...
                if (len(ent) > 0):
                    TRAIN_DATA.append((obj_query.query,{'entities' : ent}))
            ...
            if entity is None:
                ...
                if flag:
                    entity_data = {  
                        'id' : id,                  
                        'query' : query,
                        'entity_name' : request.POST['entity'], #entity_list[i+2],
                        'start_pos' : start_pos,
                        'end_pos' : end_pos,
                        'is_trained' : False
                    } 
